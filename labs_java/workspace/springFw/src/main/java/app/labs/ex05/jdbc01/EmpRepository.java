...
public class EmpRepository implements IEmpRepository {
	...
	@Autowired
	JdbcTemplate jdbcTemplate; // preparedStatment 기반 -> 동적 SQL 캐싱
	
	// table column과 seter Mapping class
	// 해암다 실행되는 mapRow 메소드 사용 가능
    private class EmpMapper implements RowMapper<EmpVO> { // rowmapper는 DB 한 행을 VO에 매핑해주는 기능 구현
        @Override
        public EmpVO mapRow(ResultSet rs, int rowNum) throws SQLException {
            EmpVO emp = new EmpVO(); // 1행마다 개체를 만듦
